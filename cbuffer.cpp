...
class CommBuffer {
 private:
  ...
  static const uint8_t MAX_BUFFER_LENGTH =
      sizeof(uint8_t);  // We should never send more than 256 bytes at once
   ...
   * Byte 5 to n: Payload
   */
  uint8_t serialize(MsgType type, void* payload, uint8_t size,
                    uint8_t* buffer) {
  ...
  Message* deserialize() {
    ...
    msg->payload =
        malloc(msg->size);  // Cause "new void[SIZE];" doesn't work ;(
 ...
 public:
  ...
  bool hasMessage() { return msgReady; }
